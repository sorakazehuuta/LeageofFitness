            ...
            for (int i = 0; i < obj.transform.childCount; i++)
            {
                if (obj.name == "Dynamic")
                {
                    if (obj.transform.GetChild(i).gameObject.GetComponent<VRC.Udon.UdonBehaviour>() != null)
                    {
                        ...
                        // VketUdonControlが参照エラー
                        // 今回はリストラされているのでいったんコメントアウト
                        if (!excludedAssetGUIDs.Contains(GetGUID(prefab)) /* && obj.transform.GetChild(i).gameObject.GetComponent<Vket.UdonManager.VketUdonControl>() == null */)
                        {
                            errorFlag = true;
