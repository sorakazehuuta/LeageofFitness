...
namespace VitDeck.Exporter
{
    ...
    public static class Exporter
    {
        ...
        {
            if (string.IsNullOrEmpty(baseFolderPath)) return new string[] { };

            var regex = new Regex(baseFolderPath + @"/[0-9]+/ReflectionProbe(-[0-9]+)?\.exr$");
            var paths = AssetDatabase.FindAssets("t:Object", new string[] { baseFolderPath })
                .Select(guid => AssetDatabase.GUIDToAssetPath(guid))
                .Where( path => !regex.IsMatch(path));   // 自動生成されたRefrectionProbeを削除する(統合時に同じものが生成されるため)
#if VRC_SDK_VRCSDK3
