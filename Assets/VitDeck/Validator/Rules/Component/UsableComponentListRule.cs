        ...
        private void AddComponentIssue(string name, GameObject obj, Component component, ValidationLevel level)
        {
            switch (level)
            {
                ...
                case ValidationLevel.DISALLOW:
                    /* {0}:{1}の使用は許可されていません。 */
                    var message = LocalizedMessage.Get("UsableComponentListRule.Disallow", name, component.GetType().Name);
                    var solution = "";
                    if (!_isItem)
                    {
                    ...
                    var solutionURL = LocalizedMessage.Get("UsableComponentListRule.Disallow.SolutionURL");
